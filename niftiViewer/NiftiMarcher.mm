...
#include "nifti1_io.h"
#include "NiftiMarcher.h"
#include "Tables.h"

NiftiMarcher::NiftiMarcher(void) {
    XDIM = 256;
    YDIM = 256;
    ZDIM = 256;
    pVolume = NULL;
...
NiftiMarcher::NiftiMarcher(const int xdim, const int ydim, const int zdim) {
    XDIM = xdim;
    YDIM = ydim;
    ZDIM = zdim;
    pVolume = NULL;
...
void NiftiMarcher::SetVolumeDimensions(const int xdim, const int ydim, const int zdim) {
    XDIM = xdim;
    YDIM = ydim;
    ZDIM = zdim;
    invDim.x = 1.0f/XDIM;
...
void NiftiMarcher::SetNumSamplingVoxels(const int x, const int y, const int z) {
    X_SAMPLING_DIST = x;
    Y_SAMPLING_DIST = y;
    Z_SAMPLING_DIST = z;
}
void NiftiMarcher::SetIsosurfaceValue(const GLubyte value) {
    isoValue = value;
}

bool NiftiMarcher::LoadVolume(const std::string& filename) {
...
void NiftiMarcher::SampleVoxel(const int x, const int y, const int z, glm::vec3 scale) {
...
size_t NiftiMarcher::GetTotalVertices() {
    return vertices.size();
}
Vertex* NiftiMarcher::GetVertexPointer() {
    return  &vertices[0];
}

GLubyte NiftiMarcher::SampleVolume(const int x, const int y, const int z) {
...
glm::vec3 NiftiMarcher::GetNormal (const int x, const int y, const int z) { 
    glm::vec3 N;
...
float NiftiMarcher::GetOffset(const GLubyte v1, const GLubyte v2) {
